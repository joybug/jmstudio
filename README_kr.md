...
### 8. 🌐 외부 모바일 기기 접속 및 보안 암호 보호 (v3.7.4 New)
...
## 🆚 Obsidian과의 차이점 (JMS vs Obsidian)

**Joy Markdown Studio(JMS)**와 **옵시디언(Obsidian)**은 둘 다 강력한 로컬 기반 마크다운 에디터라는 공통점이 있지만, 철학과 타겟 사용자층, 그리고 핵심 기능(특히 이공계 특화)에서 명확한 차이점을 가집니다.
...
* **우리 앱 (JMS)**: 설치하자마자 **수학, 물리학, 화학 연구자를 위한 완벽한 환경이 기본 내장**되어 있습니다. 마우스 클릭 한 번으로 미적분, 극한, 로런츠 힘, 슈뢰딩거 방정식 같은 복잡한 템플릿을 즉시 삽입할 수 있는 전용 헬퍼 패널이 제공됩니다.

### 2. 🧬 PubChem 화학 구조식 실시간 연동 및 시각화 (JMS의 킬러 기능)
* **옵시디언**: 화학 분자식을 검색하거나 렌더링하는 기능이 기본적으로 없습니다.
* **우리 앱 (JMS)**: 미국 국립의학도서관(NLM)의 **PubChem API가 기본 연동**되어 있습니다. 한글로 "아스피린", "카페인"을 검색하면 실시간으로 2D 분자 구조를 보여주고, SMILES 코드를 자동으로 삽입하여 본문에 분자 구조 그래픽을 렌더링해 줍니다. 
...
* **우리 앱 (JMS)**: 실행 시 자동으로 내부 웹 서버가 구동되므로, 동일한 와이파이에 있는 태블릿이나 스마트폰에서 브라우저로 `http://192.168.x.x:58220`에 접속하면 **곧바로 모바일 서재처럼 접근**할 수 있습니다. 자체적인 암호 설정(Lock Screen)까지 지원하여 보안도 완벽합니다.

### 4. 🎨 글래스모피즘(Glassmorphism) 기반의 프리미엄 UI
* **옵시디언**: 기본 UI가 투박하며, 예쁘게 꾸미려면 CSS 테마를 직접 만져야 합니다.
* **우리 앱 (JMS)**: 모던 웹 기술을 활용한 아름다운 반투명 글래스모피즘 UI와 부드러운 애니메이션을 기본 탑재하여, 연구를 작성하는 내내 시각적인 즐거움과 고급스러운 경험을 제공합니다.

### 5. 🖨️ 지능형 PDF 인쇄 및 독립형(Standalone) HTML 내보내기
* **옵시디언**: PDF 변환 시 어두운 테마가 그대로 인쇄되거나 설정이 번거롭습니다.
* **우리 앱 (JMS)**: 다크 모드에서 인쇄 버튼을 누르더라도, 잉크 절약과 가독성을 위해 **자동으로 0.1초 만에 화이트 테마로 렌더링하여 PDF를 생성**하고 곧바로 복귀합니다. 또한 모든 디자인이 압축되어 포함된 단일 파일 HTML 내보내기를 지원하여 공유가 극도로 쉽습니다.
